\documentclass[11pt, a4paper]{article}
\usepackage[a4paper, margin=1in]{geometry}
\usepackage[english]{babel}
\usepackage{blindtext}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{epsfig}
\usepackage{graphicx}
\usepackage{float}

\begin{document}

\begin{titlepage}
  \centering
  \title{ROTA \\\large Remotely Operated Transportation Apperatus \\\small \textbf{Rijksuniversiteit Groningen}}
  \author{Bastiaan van Houttum - S3146391 \\ Oliver Strik - S3100693}
  \date{November 2017}
  \maketitle
  \thispagestyle{empty}
  %\includegraphics[width=0.5\textwidth]{images\RUGLOGO.eps}\\[0.5cm]
\end{titlepage}

\pagebreak\tableofcontents\pagebreak
% Sections
\input{sections/probDesc/problemDescription}
\input{sections/probAnal/problemAnalysis}
\input{sections/design/design}
\input{sections/progCode/programCode}
\input{sections/testRes/testResults}
\input{sections/eval/evaluation}
\input{bib}
%\pagebreak\input{references}
%\pagebreak\input{appendix}
\end{document}
